// webgl_patch.js - Browser-injectable WebGL anti-fingerprinting patch
(function() {
  console.log('[Obscura] WebGL patch script injected');
  ...
  function getWebGLConfig(persona) {
    // You can extend this with more config if needed
    return (persona && persona.webgl) ? persona.webgl : { enabled: true };
  }
  ...
  function installReadbackNoise(gl, prng) {
    ...
    gl.readPixels = function(x, y, width, height, format, type, pixels) {
      ...
      // Only apply noise if draw noise is NOT present
